...
**Conservative box width (Claude Code padding workaround).** Claude Code does not expose its statusline area's usable width to the command — `process.stdout.columns` reports the full terminal, but the actual area is narrower because Claude Code reserves padding / indicator space. The request to fix this upstream was [filed and closed as not planned](https://github.com/anthropics/claude-code/issues/5430). As a client-side workaround, the statusline reserves 4 columns by default so the box never overflows; if your terminal renders edge-to-edge or you still see truncation, set `statuslineWidthOffset` in `~/.claude/cc-statusline-rows.json` to fine-tune.
