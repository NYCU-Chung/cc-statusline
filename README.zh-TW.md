...
**保守 box 寬度（Claude Code padding workaround）。** Claude Code 沒給 statusline 它真實的可用寬度 — `process.stdout.columns` 拿到的是整個 terminal 寬度，但實際 statusline 區域比較窄（Claude Code 自己留 padding / indicator 空間）。upstream 的 request [已被標 closed as not planned](https://github.com/anthropics/claude-code/issues/5430)。Client-side workaround 是預設保留 4 columns 給 Claude Code、避免 box 超寬被截。如果你的 terminal 是邊到邊渲染、或還是看到被截，可以在 `~/.claude/cc-statusline-rows.json` 設 `statuslineWidthOffset` 微調。
